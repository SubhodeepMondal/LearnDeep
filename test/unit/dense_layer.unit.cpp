...
TEST_F(FrameworkTest, DenseLayer_Test_3) {
  ...
  call_back.record_tensor_loss(loss_sgd, false);
  ...
  for (auto output : outputs) {
    for (int j = 0; j < 255; j++)
      for (int i = 0; i < 3; i++)
        EXPECT_NEAR(output[0].getData()[i + j * 3],
                    dense_layer_Test_3_output_data[i + j * 3], 1e-6)
            << "at: " << i + j * 3;
  }
  ...
  for (unsigned ep = 0; ep < epoch; ep++)
    for (int j = 0; j < 255; j++)
      for (int i = 0; i < 3; i++)
        EXPECT_NEAR(training_losses[ep][0].getData()[i + j * 3],
                    dense_layer_Test_3_loss_data[i + j * 3], 1e-5)
            << "at i: " << i << " j: " << j;
}