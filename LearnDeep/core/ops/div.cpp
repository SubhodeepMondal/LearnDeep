...
void Opsdiv::addGradGraph(Graph *gradient_graph) {
...
void Opsdiv::initializeinputs(Tensor<std::float64_t> **inputs) {
  unsigned i;
...
void Opsdiv::initializeoutput(Tensor<std::float64_t> *output) {
  this->output = output;
  *(this->output) = *(inputs[0]);
}

void Opsdiv::printinputs() {
  unsigned i;
  for (i = 0; i < 2; i++) {
    std::cout << "Input: " << i << "\n";
    inputs[i]->printData();
...
void Opsdiv::kernel_dispatch(std::float64_t **ptr, const unsigned nDimA,
