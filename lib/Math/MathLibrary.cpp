...
void NDMath<T, typeFlag>::recursive_iterator(
    unsigned index, unsigned *dimension_arr, NDMath<T, typeFlag> input,
    NDMath<T, typeFlag> &output, void (*__kernel)(double **, unsigned *),
    ...
    NDMath<T, typeFlag> misc_arr)
{
  if (index < 2)
  {
    ...
    inpA_x = (this->getNoOfDimensions() > 0)
                 ? NDMath<T, typeFlag>::getDimensions()[0]
                 : 1;
    inpA_y = (this->getNoOfDimensions() > 1)
                 ? NDMath<T, typeFlag>::getDimensions()[1]
                 : 1;
    ...
    // std::cout << "a index: " << a_plane_size << " b index: " << b_plane_size
    // << " c index: " << c_plane_size << "\n";
    if (input.getNoOfDimensions() > 2)
      for (i = 2; i < input.getNoOfDimensions(); i++)
      {
        ...
        // std::cout << "a index: " << a_index << " b index: " << b_index << " c
        // index: " << c_index << "\n";
      }

    // std::cout << "a index: " << a_index << " b index: " << b_index << " c
    // index: " << c_index << "\n"; std::cout << input.getData() << "  " <<
    // output.getData() << "\n";
...
template <typename T, int typeFlag>
void NDMath<T, typeFlag>::recursive_sum(unsigned index, unsigned *dimension_arr,
    ...
    if (this->getDimensions()[0] == input.getDimensions()[1])
    {
      ...
      if (flag)
      {
        ...
        recursive_iterator(this->getNoOfDimensions() - 1, dimension_arr, input,
                           output, cpu::__mmul, "matrix_multiplication", NULL,
                           NULL, NULL);
...
NDMath<T, typeFlag> NDMath<T, typeFlag>::scalermul(const double scalerFactor)
{
  ...
  {
    cpu::__mscalermul(this->getData(), scalerFactor, output->getData(), dim_x,
                      dim_y);
  }
  else
  {
    ...
    for (unsigned i = 2; i < no_of_dims; i++)
      ...
      {
        cpu::__mscalermul(this->getData() + plan_size, scalerFactor,
                          this->getData(), dim_x, dim_y);
...
template <typename T, int typeFlag>
NDMath<T, typeFlag>
NDMath<T, typeFlag>::operator*(const NDMath<double, 0> input)
{
  NDMath<T, typeFlag> output;
  unsigned i, j, no_of_dimensions, flag = 1;
  unsigned a_plan_dim, b_plan_dim, c_plan_dim;
  ...
  if (no_of_dimensions == input.getNoOfDimensions())
  {
    ...
    if (flag && this->getDimensions()[0] == input.getDimensions()[1])
    {
      ...
      if (no_of_dimensions < 3)
      {
        ...
        cpu::__mmul(NDMath<T, typeFlag>::getData(), input.getData(),
      ...
      else
      {
        ...
        for (i = 2; i < no_of_dimensions; i++)
        {
          ...
          {
            cpu::__mmul(NDMath<T, typeFlag>::getData() + a_actual_index,
                        input.getData() + b_actual_index,
                        output.getData() + c_actual_index, dim_x, dim_y, dim_z);
...
NDMath<T, typeFlag> NDMath<T, typeFlag>::add(const NDMath<double, 0> input)
{
  ...
  if (flag)
  {
    ...
    output =
        NDMath<T, typeFlag>(this->getNoOfDimensions(), this->getDimensions());
...
template <typename T, int typeFlag>
NDMath<T, typeFlag>
NDMath<T, typeFlag>::vectoradd(const NDMath<double, 0> input)
{
...
NDMath<T, typeFlag>::operator-(const NDMath<double, 0> input)
{
  ...
  if (flag)
  {
    ...
    recursive_iterator(this->getNoOfDimensions() - 1, dimension_arr, input,
                       output, cpu::__msub, "matrix_multiplication", NULL, NULL,
                       NULL);
...
void NDMath<T, typeFlag>::reducesum(NDMath<T, typeFlag> &output, first_dim n,
                                    Args... args)
{
