...
void Opsmul::compute() {
  ...
  recursive_iterator(inputs[0]->getNoOfDimensions() - 1, arr, *inputs[0],
                     *inputs[1], *output, "matrix_element_wise_multiplication",
                     NULL, NULL, NULL);

  delete[] arr;
}

void Opsmul::initilizeinputs(tensor<double> **inputs, unsigned no_of_inputs) {
  unsigned i;
...
void Opsmul::initilizeoutput(tensor<double> *output) {
  this->output = output;
