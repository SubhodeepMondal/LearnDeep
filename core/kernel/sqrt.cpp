...
void Opssqrt::recursive_iterator(unsigned index, unsigned *dimension_arr,
...
void Opssqrt::initilizeinputs(Tensor<std::float64_t> **inputs,
                              unsigned no_of_inputs) {
  this->inputs = new Tensor<std::float64_t> *[1];
  this->inputs[0] = inputs[0];
}

void Opssqrt::initilizeoutput(Tensor<std::float64_t> *outputs) {
  this->output = outputs;
  *(this->output) = *(inputs[0]);
}

void Opssqrt::printinputs() {
  unsigned i;
  for (i = 0; i < 1; i++) {
    std::cout << "Input: " << i << "\n";
    inputs[i]->printData();
...
void Opssqrt::printoutput() {
  std::cout << "output:\n";
  output->printData();
  std::cout << "\n";
}

void Opssqrt::kernel_dispatch(std::float64_t **ptr, unsigned *arr) {
#ifdef CUDA_ENABLED
  double *d_arr[2];
  d_arr[0] = reinterpret_cast<double *>(ptr[0]);
  d_arr[1] = reinterpret_cast<double *>(ptr[1]);

  gpu::gpu_mat_sqrt_f64(d_arr, arr);
#else
  if (__builtin_cpu_supports("avx2")) {
    avx2::avx2_sqrt_f64(ptr, arr);
  } else {
    cpu::__msqrt(ptr, arr);
  }
#endif
