...
void Opsadd::compute() {
  ...
  recursive_iterator(inputs[0]->getNoOfDimensions() - 1, arr, *inputs[0],
                     *inputs[1], *output, "matrix_addition", NULL, NULL, NULL);

  delete[] arr;
}

void Opsadd::initilizeinputs(tensor<double> **inputs, unsigned no_of_inputs) {
  unsigned i;
...
void Opsadd::initilizeoutput(tensor<double> *output) {
  this->output = output;
