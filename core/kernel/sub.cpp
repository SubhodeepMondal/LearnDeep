...
void Opssub::recursive_iterator(unsigned index, unsigned *dimension_arr,
...
void Opssub::kernel_dispatch(std::float64_t **ptr, unsigned *arr) {
  ...
  gpu::gpu_mat_sub_f64(d_arr, arr);
#else
  if (__builtin_cpu_supports("avx2")) {
    avx2::avx2_sub_f64(ptr, arr);
  } else {
    cpu::__msub(ptr, arr);
  }
#endif
