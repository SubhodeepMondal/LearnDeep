...
// This function multiplies each element of a matrix of type double by a scalar value and stores the result in a second matrix.
// The matrix is represented as a pointer to a double array, and its dimensions are specified by the
// parameters in the 'arr' array, where arr[0] is the number of rows and arr[1] is the number of columns.
// The scalar value is provided as a double.
// The function uses CUDA to perform the multiplication on the GPU.
// It allocates memory on the GPU for the input matrix and the output matrix,
// copies the input matrix from the host to the device, launches a CUDA kernel to perform the multiplication,
// and finally copies the result back to the host before freeing the GPU memory.
// The function also checks for CUDA errors and prints an error message if any occur.
// Parameters:
// - ptr: An array of pointers to double arrays, where ptr[0] is the input matrix,
//        ptr[1] is the result matrix, and ptr[2] is the scalar value (as a single-element array).
// - arr: An array of unsigned integers where arr[0] is the number of rows and arr[1] is the number of columns of the matrix.   
